...
HYPRPAPER_CONFIG=${HYPRPAPER_CONFIG:-"$HOME/.config/hypr/hyprpaper.conf"}
...
select_backend() {
  ...
  log "hyprpaper not available in PATH; install hyprpaper to continue."
  exit 1
}

ensure_hyprpaper_config() {
  local config_dir
  config_dir=$(dirname "$HYPRPAPER_CONFIG")
  mkdir -p "$config_dir"
  ...
  local fallback_image="${IMAGES[0]}"
  {
    echo "# Autogenerated by dynamic-wallpaper.sh"
    echo "preload = $fallback_image"
    echo "wallpaper = ,$fallback_image"
  } >"$HYPRPAPER_CONFIG"

  log "Created hyprpaper config at $HYPRPAPER_CONFIG"
}
...
apply_wallpaper() {
  ...
  if ! hyprctl_cmd hyprpaper preload "$image" >/dev/null 2>&1; then
    log "Failed to preload $image for $monitor"
    return 1
  fi

  if ! hyprctl_cmd hyprpaper wallpaper "$monitor,$image" >/dev/null 2>&1; then
    log "Failed to set wallpaper $image on $monitor"
    return 1
  fi
